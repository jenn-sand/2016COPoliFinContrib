...
```{r map_retiree_status, include=FALSE}
...
```{r echo = FALSE, Bivariate_Funding_Density}
...
```{r echo = FALSE, Retiree_Status_Amount_difference}
...
p <- plot_ly(donations_by_party, labels = ~cand_party, values = ~total_donation, type = 'pie',
          textposition = 'inside',
          textinfo = 'label+percent',
          insidetextfont = list(color = "#FFFFFF"), #white to make it pop
          hoverinfo = 'text',
          text = ~paste(cand_party, '$', total_donation, 'dollars'),
          marker = list(colors = colors,
                        line = list(color = "#FFFFFF, width = 1")),
          showlegend = FALSE) %>% 
  layout(title = 'United States Colorado Political Donations by Party in 2016 Election',
         #showgrid = FALSE hides the grid. zeroline is the 0,0 line,
            #ticklabels are the numbers on grid.
         xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))

ggplotly(p, filename = "pie/styled")
